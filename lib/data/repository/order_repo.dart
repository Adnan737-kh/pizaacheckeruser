...
class OrderRepo {
  ...
  Future<ApiResponse> getDistance(String distance) async {
    try {
      final response = await dioClient!.get('https://backend.pizzachecker.net/api/v1/products/get_minimum_amount/$distance');
  ...
  Future<ApiResponse> getOrderList() async {
    try {
      ...
      print('order list $response.data');
  ...
  Future<ApiResponse> orderDetailsWithPhoneNumber(String? orderID, String phoneNumber) async {
    try {
      ...
      print('order deta ${response.data}');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponse> placeOrder(PlaceOrderBody orderBody, {String? guestId}) async {
    try {
      ...
      print('response place order ${response.data}');
      print('response place order ${response.statusCode}');
      print('response place order ${response.statusMessage}');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
