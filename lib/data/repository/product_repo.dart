...
class ProductRepo {
  ...
  Future<ApiResponse> getLatestProductList(String offset) async {
    try {
      ...
      // print('lates products ${response.data}');
  ...
  Future<ApiResponse> getProductSubCategories(int productId) async {
    try {
      final response = await dioClient!.get(
        'https://backend.pizzachecker.net/api/v1/products/get_variant/$productId',
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
