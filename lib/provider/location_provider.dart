...
class LocationProvider with ChangeNotifier {
  ...
  void checkPermission(Function callback, BuildContext context) async {
    ...
    if(permission == LocationPermission.denied) {
      permission = await Geolocator.requestPermission();
        ...
        showDialog(context: Get.context!, barrierDismissible: false, builder: (context) =>
  ...
  void deleteUserAddressByID(int? gustIndex,int? id, int index, Function callback) async {
    ApiResponse apiResponse = await locationRepo!.removeAddressByID(id??gustIndex);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      print('Address response ${apiResponse.response?.data}');
  ...
  Future<ResponseModel> addAddress(AddressModel addressModel) async {
    ...
    final body = jsonEncode(addressModel.toJson());

    print('res prov ${apiResponse.response?.data}');
    print('address body $body'); // Print the body to debug
  ...
  void saveSelectedLocation() async {
    ...
    prefs.setDouble('selectedLatitude', _position.latitude);
    prefs.setDouble('selectedLongitude', _position.longitude);
    // You can also save other details like address if needed
    prefs.setString('selectedAddress', _address ?? '');
  }
      ...
      if (text.isNotEmpty) {
        ...
        if (response.response != null && response.response!.statusCode == 200) {
          ...
          if (responseData != null && responseData['status'] == 'OK') {
            ...
            // print('Error: Response status is not OK or response data is null');
            ApiChecker.checkApi(response);
          }
        } else {
          // print('Error: Response status code is not 200 or response is null');
          ApiChecker.checkApi(response);
        }
      }
    } catch (e) {
