...
class CustomerCouponProvider extends ChangeNotifier {
  final CouponRepo? couponRepo;
  CustomerCouponProvider({required this.couponRepo});
  ...
  Future<void> getCustomerCouponList() async {
    ApiResponse apiResponse = await couponRepo!.getCustomerCouponList(
      guestId: Provider.of<AuthProvider>(Get.context!, listen: false).getGuestId(),
    );
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
