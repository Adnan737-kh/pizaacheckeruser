...
class OrderDetailsScreen extends StatefulWidget {
  ...
  const OrderDetailsScreen({Key? key, required this.orderModel,
    required this.orderId, this.phoneNumber}) : super(key: key);
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen> {
  ...
  void _loadData(BuildContext context) async {
    ...
    ResponseModel? response = await orderProvider.trackOrder(widget.orderId.toString(),
        orderModel: widget.orderModel,fromTracking: false, phoneNumber: widget.phoneNumber);
    ...
    await orderProvider.getOrderDetails(widget.orderId.toString(),
        phoneNumber: widget.phoneNumber, isApiCheck: response != null && response.isSuccess);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: (ResponsiveHelper.isDesktop(context)
          ...
          title: getTranslated('order_details', context))) as PreferredSizeWidget?,
      ...
      body: Consumer<OrderProvider>(
        builder: (context, order, child) {
          ...
          if( order.trackModel != null &&
              order.trackModel!.extraDiscount != null && order.trackModel?.id != -1) {
            extraDiscount  = order.trackModel!.extraDiscount ?? 0.0;
          }
          double subTotal = itemsPrice + tax + addOns;
          double couponAmount = order.trackModel != null
              && order.trackModel?.id != -1 ?  order.trackModel?.couponDiscountAmount ?? 0 : 0;
          double total = itemsPrice + addOns - discount
              - extraDiscount + tax + deliveryCharge! - couponAmount;

          return !order.isLoading && order.orderDetails != null
              && order.trackModel != null  ? order.orderDetails!.isNotEmpty ?
