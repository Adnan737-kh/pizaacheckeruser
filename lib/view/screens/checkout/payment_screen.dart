...
class PaymentScreen extends StatefulWidget {
  ...
  const PaymentScreen({Key? key, this.url,
    this.formCheckout = true}) : super(key: key);
  ...
  void _pageRedirect(String url) {
    if(_canRedirect) {
      ...
      if(isSuccess){
        ...
        if(isWallet){
           RouterHelper.getWalletRoute(true, token: token, flag: 'success', action: RouteAction.pushReplacement);
        }else{
          if(token.isNotEmpty) {
            ...
            _placeOrderBody =  PlaceOrderBody.fromJson(jsonDecode(placeOrderString)).copyWith(
              ...
              transactionReference:  transactionReference.replaceRange(0,
