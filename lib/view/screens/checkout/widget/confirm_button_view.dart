...
class ConfirmButtonView extends StatelessWidget {
  ...
  const ConfirmButtonView({Key? key,
              ...
              builder: (context) => CustomButton(
                backgroundColor: backgroundColor ,
                  ...
                  onTap: () {
                    ...
                    if(orderAmount < distanceMinimumAmount){}else {
                      ...
                      if(orderProvider.selectedPaymentMethod != null || orderProvider.selectedOfflineValue != null){
                        ...
                        if(orderAmount < configModel.minimumOrderValue!) {
                        ...
                        } else if(!takeAway && (locationProvider.addressList == null
                            || locationProvider.addressList!.isEmpty || orderProvider.addressIndex < 0)) {
                          ...
                          for (int index = 0; index < cartList.length; index++) {
                            ...
                            carts.add(Cart(
                              ...
                              cart.discountAmount, cart.quantity, cart.taxAmount, addOnIdList,
                              addOnQtyList,cart.extraIds!,cart.dropDownIDs!,
                            ));
                          }
                          ...
                          PlaceOrderBody placeOrderBody = PlaceOrderBody(
                            ...
                            takeAwayInfo: orderProvider.takeAwayInfo,
                          );
                          ...
                          if(placeOrderBody.paymentMethod == 'wallet_payment'
                            ...
                            print('order_amount=${orderProvider.partialAmount}');
                            String url = "customer_id=${authProvider.getGuestId() ?? profileProvider.userInfoModel!.id}"
                                "&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                                "&&callback=${AppConstants.baseUrl}${RouterHelper.orderSuccessScreen}"
                                "&&order_amount=${orderProvider.partialAmount ?? (orderAmount + (deliveryCharge ?? 0)).toStringAsFixed(2)}";

                            String webUrl = "customer_id=${authProvider.getGuestId() ?? profileProvider.userInfoModel!.id}"
                                "&&is_guest=${authProvider.getGuestId() != null ? '1' : '0'}"
                                "&&callback=$protocol//$hostname${kDebugMode ? ':$port' : ''}${RouterHelper.orderWebPayment}"
                                "&&order_amount=${orderProvider.partialAmount ?? (orderAmount + (deliveryCharge ?? 0)).toStringAsFixed(2)}"
                                "&&status=";
