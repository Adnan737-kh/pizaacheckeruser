...
class PaymentSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print('totalam $total and ');
    return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
      print('partialAmount ${orderProvider.partialAmount}');
      ...
      return Container(
        decoration: BoxDecoration(color: Theme.of(context).cardColor, boxShadow: [BoxShadow(
          ...
          offset: const Offset(0, 4), blurRadius: 4, spreadRadius: -2,
        )]),
        padding: const EdgeInsets.symmetric(horizontal:
        Dimensions.paddingSizeDefault, vertical: Dimensions.paddingSizeDefault),
       ...
       child: Column(children: [
         Row(children: [
             orderProvider.selectedOfflineMethod != null ? Image.asset(
               ...
               image: '${configModel.baseUrls?.getWayImageUrl}'
                   '/${orderProvider.paymentMethod?.getWayImage}',)
        ...
        if(orderProvider.selectedOfflineValue != null) Padding(
          ...
          child: Column(children: orderProvider.selectedOfflineValue!.map((method) => Padding(
            ...
            child: Row(children: [
               Flexible(child: Text(method.keys.single, style: rubikRegular,
                   maxLines: 1, overflow: TextOverflow.ellipsis)),
               const SizedBox(width: Dimensions.paddingSizeSmall),

               Flexible(child: Text(' :  ${method.values.single}',
                   style: rubikRegular, maxLines: 1, overflow: TextOverflow.ellipsis)),
             ]),
          )).toList()),
        ),
       ]),
