...
class _PaymentMethodBottomSheetState extends State<PaymentMethodBottomSheet> {
  ...
  void initState() {
    ...
    if(authProvider.isLoggedIn() && walletBalance != null
        && walletBalance > 0 && walletBalance >= widget.totalPrice){
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      child: Center(child: SizedBox(width: 550,
              ...
              builder: (ctx, orderProvider, _) {
                return Column(
                  ...
                  Row(children: [
                    ...
                    configModel.walletStatus! && authProvider.isLoggedIn() &&
                        (orderProvider.partialAmount == null) && !isPartialPayment ? Expanded(
                      child: PaymentButtonNew(
                        ...
                        onTap: () {
                          if(canSelectWallet) {
                            ...
                            showDialog(context: context, builder: (ctx)=> PartialPayDialog(
                              isPartialPay: profileProvider.userInfoModel!.walletBalance!
                                  < widget.totalPrice,
                              totalPrice: widget.totalPrice,
                            ));
                          }else{
                            showCustomSnackBar(getTranslated('your_wallet_have_not_sufficient_balance',
                                context));
                          }
                        },
                      ),
                 ...
                 if(paymentList.isNotEmpty)
                   Row(children: [
                    ...
                    Text('(${getTranslated('faster_and_secure_way_to_pay_bill', context)})',
                  ...
                  Expanded(child: PaymentMethodView(
                    ...
                    onTap: (index)async {
                      ...
                      prefs.setInt('onlineIndex', index);
                      prefs.remove('selectedPaymentIndex');


                      int selectedPaymentIndex =  prefs.getInt('onlineIndex')!;
                      orderProvider.onlinePayIndex =selectedPaymentIndex;
                      print('my online pref $selectedPaymentIndex');
                      orderProvider.onlinePayIndex = index;
                      if(notHideOffline &&  paymentList[index].type == 'offline'){
                        ...
                        showCustomSnackBar('${getTranslated('you_can_not_use', context)}'
                      ...
                      if(orderProvider.paymentMethod?.type == 'offline'){
                        if(orderProvider.selectedOfflineValue != null){
                          ...
                          showDialog(context: context, builder: (ctx)=>
                              OfflinePaymentView(totalAmount: widget.totalPrice));
                        }
                      }else{
                        orderProvider.savePaymentMethod(index: orderProvider.paymentMethodIndex,
                            method: orderProvider.paymentMethod);
                        // print('index ${orderProvider.paymentMethodIndex}');
                        context.pop();
