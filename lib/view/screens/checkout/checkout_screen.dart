...
import '../../../main.dart';
import '../address/widget/delete_confirmation_dialog.dart';
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void initState() {
    ...
    // loadDeliveryFee(orderProvider,0);
    _initializeSavedAddressIndex(orderProvider);
  ...
  void _initializeSavedAddressIndex(OrderProvider orderProvider) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
     savedIndex = prefs.getInt('selectedAddressIndex') ?? -1;

    if (savedIndex != -1) {
      if (isIndexAvailable(savedIndex)) {
        // print('selected index is $savedIndex');
        orderProvider.setAddressIndex(savedIndex);
        callLoadDel(orderProvider);
      }
    }
  }


  callLoadDel(OrderProvider orderProvider){
    final LocationProvider locationProvider = Provider.of<LocationProvider>(context, listen: false);
    loadDeliveryFee(orderProvider,
      savedIndex
      ,locationProvider,
  ...
  double deliveryChargePerKm = 0;

  void _showDeliveryFeeDialog(OrderProvider orderProvider ) {
    showDialog(
      context: context,
  ...
  Future<void> loadDeliveryFee(
    ...
    // print('the data is $index  $shippingPerKm $minShippingCharge');
    final isSuccess = await orderProvider.getDistanceInMeter(
      LatLng(
    ...
    if (isSuccess) {
      final distance = orderProvider.distance;
      // print('The distance is $distance');


      double updatedDistance = distance;

      int fixedDistance = distance.round();
      miniAmount = await orderProvider.getDistanceMinimumAmount(fixedDistance.toStringAsFixed(2));
      print('distance is ${fixedDistance.toStringAsFixed(2)}');

        if (updatedDistance > 2) {
          // Calculate delivery charge based on distance
          deliveryChargePerKm = updatedDistance * shippingPerKm;
          if (deliveryChargePerKm < minShippingCharge) {
            deliveryChargePerKm = minShippingCharge;
          }
        } else {
          // Handle case when distance is less than or equal to 2
        }


    } else {
      // Handle case when fetching distance fails
  ...
  Future<void> loadPaymentMethods() async {
    ...
    // Check if online index is available
    int? selectedOnlinePaymentIndex = prefs.getInt('onlineIndex');
    if (selectedOnlinePaymentIndex != null) {
      // Online index is available
      orderProvider.onlinePayIndex = selectedOnlinePaymentIndex;
      // Load online payment method
      paymentList.addAll(configModel.activePaymentMethodList ?? []);
      orderProvider.changePaymentMethod(digitalMethod: paymentList[selectedOnlinePaymentIndex]);
      orderProvider.savePaymentMethod(index: null, method: orderProvider.paymentMethod, isUpdate: false);
      orderProvider. setPaymentIndex(null);

      // Remove offline index if exists
      prefs.remove('selectedPaymentIndex');
    } else {
      // Online index is not available, check for offline index
      int? selectedPaymentIndex = prefs.getInt('selectedPaymentIndex');
      if (selectedPaymentIndex != null) {
        // Offline index is available
        orderProvider.offlinePayIndex = selectedPaymentIndex;
        // Load selected payment method
        orderProvider. savePaymentMethod(index: selectedPaymentIndex, method: null, isUpdate: false);

        // Remove online index if exists
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: (ResponsiveHelper.isDesktop(context)
          ...
          title: getTranslated('checkout', context))) as PreferredSizeWidget?,
      ...
      body: _isLoggedIn || splashProvider.configModel!.isGuestCheckout! ?  Column(
        children: [
          Expanded(child: CustomScrollView(slivers: [
              ...
              SliverToBoxAdapter(
                  child: Consumer<LocationProvider>(builder: (context,
                  locationProvider, _) {
                return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
                  ...
                  if(!takeAway && kmWiseCharge) {
                    ...
                    deliveryChargePerKm =deliveryCharge;
                  ...
                  return Center(child: Container(
                    margin: EdgeInsets.symmetric(vertical: ResponsiveHelper.isDesktop(context)
                    ...
                    child: Column(children: [
                      ...
                      if(splashProvider.isBranchSelectDisable()) Row(
                        children: [
                         ...
                         if(ResponsiveHelper.isDesktop(context)) const Expanded(flex: 4,
                             child: SizedBox())
                        ],
                      ),
                      ...
                      Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Expanded(flex: 6, child: Column(
                            ...
                            Padding(
                              ...
                              child: Row(children: [
                                ...
                                SizedBox(height: 30, child: FloatingActionButton.extended(
                                  ...
                                  onPressed: () {
                                    RouterHelper.getAddAddressRoute('checkout', 'add',
                                            AddressModel());

                                  },
                                  icon: Icon(Icons.add_circle_outline, size:
                                  Dimensions.paddingSizeDefault,
                                      color: Theme.of(context).primaryColor),
                                  label: Text(getTranslated('add_delivery_info', context)!,
                                    ...
                                    _initializeSavedAddressIndex(orderProvider);
                                      loadDeliveryFee(orderProvider,
                                        ...
                                        Provider.of<SplashProvider>(context,
                                            listen: false).configModel!.deliveryManagement!.shippingPerKm!,
                                        Provider.of<SplashProvider>(context,
                                            listen: false).configModel!.deliveryManagement!.minShippingCharge!,
                                      );
                                    dialogShown = true;
                                  }
                                  return Padding(
                                    ...
                                    child: InkWell(
                                      onTap: () async {
                                        if(isAvailable) {
                                          ...
                                          loadDeliveryFee(orderProvider, index ,
                                            locationProvider,
                                            Provider.of<SplashProvider>(context,
                                                listen: false).configModel!.deliveryManagement!.shippingPerKm!,
                                            Provider.of<SplashProvider>(context,
                                          ...
                                          if(kmWiseCharge) {
                                            if(orderProvider.selectedPaymentMethod != null){
                                              showCustomSnackBar(getTranslated('your_payment_method_has_been',
                                                  context), isError: false);
                                            }
                                            showDialog(context: context, builder: (context) =>
                                                Center(child: Container(height: 100, width: 100, decoration:
                                            ...
                                            orderProvider.getDistanceInMeter(
                                              ...
                                              context.pop();
                                              if(isSuccess) {
                                                _showDeliveryFeeDialog(orderProvider);

                                              }else {
                                                showCustomSnackBar(
                                                    getTranslated('failed_to_fetch_distance', context));
                                              }
                                              return isSuccess;
                                            });
                                      ...
                                      child: Stack(children: [
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            borderRadius: BorderRadius.circular(10),
                                            border: index == orderProvider.addressIndex
                                          ...
                                          child: Row(children: [
                                            ...
                                            Expanded(
                                              ...
                                              Column(crossAxisAlignment: CrossAxisAlignment.start,
                                                ...
                                                Text(locationProvider.addressList![index].address!,
                                                    style: rubikRegular, maxLines: 1,
                                                  ...
                                                  onSelected: (String result) {
                                                    if (result == 'delete') {
                                                      showDialog(context: context, barrierDismissible: false,
                                                          builder: (context) =>
                                                              DeleteConfirmationDialog(addressModel:
                                                              locationProvider.addressList![index],index: index,));
                                                    } else {
                                                      Provider.of<LocationProvider>(context,
                                                          listen: false).updateAddressStatusMessage(message: '');
                                                      RouterHelper.getAddAddressRoute('address', 'update',
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                return Row(mainAxisAlignment: MainAxisAlignment.start, children: [
                                  ...
                                  Text(index == 0 ? getTranslated('today', context)! :
                                  getTranslated('tomorrow', context)!, style: rubikRegular.copyWith(
                          ...
                          SizedBox(
                            height: 40,
                            child: orderProvider.timeSlots != null ? orderProvider.timeSlots!.isNotEmpty ?
                            ListView.builder(
                              scrollDirection: Axis.horizontal,
                              shrinkWrap: true,
                              physics: const BouncingScrollPhysics(),
                              ...
                              itemBuilder: (context, index) {
                                return SlotWidget(
                                  title: (
                                      index == 0 && orderProvider.selectDateSlot == 0
                                          && Provider.of<SplashProvider>(context,
                                          listen: false).isRestaurantOpenNow(context))
                          ...
                          PaymentSection(total: (widget.amount ?? 0) + (deliveryCharge)),
                          if(ResponsiveHelper.isDesktop(context)) const
                          SizedBox(height: Dimensions.paddingSizeDefault),

                          PartialPayView(totalPrice: (widget.amount  ?? 0) + (deliveryCharge)),
                          ...
                          if (!ResponsiveHelper.isDesktop(context))
                            CostSummeryView(
                              ...
                              distanceMinimumAmount:miniAmount ,
                              walletPay: orderProvider.walletUsedAmount,
                              remainingPay: orderProvider.partialAmount != null
                                  ? orderProvider.partialAmount!
                                  : null,
                            ),

                          if (widget.amount! < miniAmount)
                            Center(
                              child: Text('${getTranslated('minimum_order_amount_should_be', context)} '
                                  '${PriceConverter.convertPrice(miniAmount)}'
                                ,style: rubikMedium.copyWith(
                          ...
                          child: Container(
                            ...
                            child: Column(children: [
                              ...
                              if(ResponsiveHelper.isDesktop(context))  ConfirmButtonView(
                                ...
                                deliveryCharge: deliveryChargePerKm,
                                backgroundColor: (widget.amount! < miniAmount)
            ...
            Consumer<OrderProvider>(
              builder: (context, orderProvider, _) {
                ...
                return Container(
                  decoration: BoxDecoration(
                    color: Theme.of(context).cardColor,
                    boxShadow: [BoxShadow(color: Theme.of(context).primaryColor.withOpacity(0.1),
                        blurRadius: 10)],
                  ),
                  child: Column(children: [
                    const SizedBox(height: Dimensions.paddingSizeExtraSmall),
