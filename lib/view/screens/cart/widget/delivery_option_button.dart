  ...
  Widget build(BuildContext context) {
    return Consumer<OrderProvider>(
      builder: (context, order, child) {
        return InkWell(
          onTap: () {
            order.setOrderType(value, notify: true);
            if(value == 'take_away'){
              //
              Navigator.push(context,
                  MaterialPageRoute(builder: (context) =>   const TakeAwayInfo(),));
            }
            // print(value);
          },
          child: Row(
            children: [
              ...
              kmWiseFee ? const SizedBox() : CustomDirectionality(
                child: Text('(${value == 'delivery'
                    ? PriceConverter.convertPrice(Provider.of<SplashProvider>(context, listen: false)
                    .configModel!.deliveryCharge)
                    : getTranslated('free', context)})', style: rubikMedium),
              ),
