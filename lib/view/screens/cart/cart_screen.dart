...
class _CartScreenState extends State<CartScreen> {
  ...
  void initState() {
    ...
    Provider.of<OrderProvider>(context, listen: false).setOrderType(
      Provider.of<SplashProvider>(context, listen: false).configModel!.homeDelivery!
          ? 'delivery' : 'take_away', notify: false,
    );

    // final OrderProvider orderProvider = Provider.of<OrderProvider>(context, listen: false);
    //
    // orderProvider.getDistanceMinimumAmount(orderProvider.distance);
    // print('m am${orderProvider.distance}');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      const PreferredSize(preferredSize: Size.fromHeight(100),
          child: WebAppBar())
          :CustomAppBar(context: context,
          onBackPressed: (){
        Navigator.push(context,
            MaterialPageRoute(builder: (context) =>  const DashboardScreen(pageIndex: 0),));
      },
          title: getTranslated('my_cart', context),
          isBackButtonExist: true)) as PreferredSizeWidget?,
      body: Consumer<OrderProvider>(
          builder: (context, order, child) {
            // order.getDistanceMinimumAmount(order.distance);
            // print('m am${order.minimumAmount}');
            return Consumer<CartProvider>(
              builder: (context, cart, child) {
                ...
                for (var cartModel in cart.cartList) {
                  ...
                  // extra = (Provider.of<ProductProvider>(context,
                  //     listen: false).extraPrice * cartModel.quantity!);

                  // print('extras Price ${cartModel.extraPrice}');
                  print('drop ids ${cartModel.dropDownIDs}');
                  print('extras ids ${cartModel.extraIds}');
                              ...
                              children: [
                                ...
                                Expanded(child: Container(
                                  ...
                                  margin: ResponsiveHelper.isDesktop(context)
                                      ? const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeSmall,vertical: Dimensions.paddingSizeLarge) : const EdgeInsets.all(0),
                                  padding: ResponsiveHelper.isDesktop(context)
                                      ? const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge,vertical: Dimensions.paddingSizeLarge) : const EdgeInsets.all(0),
                                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                                ...
                                                if(_couponController.text.isNotEmpty && !coupon.isLoading) {
                                                  if(coupon.discount! < 1) {
                                                    coupon.applyCoupon(_couponController.text, total).then((discount) {
                                                      if (discount! > 0) {
                                                        showCustomSnackBar('You got '
                                                            '${PriceConverter.convertPrice(discount)} discount', isError: false);
                                                      } else {
                                    ...
                                    Provider.of<SplashProvider>(context, listen: false).configModel!.homeDelivery!?
                                    DeliveryOptionButton(value: 'delivery',
                                    ...
                                    DeliveryOptionButton(value: 'take_away',
                                    ...
                                    ItemView(
                                      title: getTranslated(kmWiseCharge ? 'subtotal'
...
class CheckOutButtonView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ((configModel.selfPickup ?? false) || (configModel.homeDelivery ?? false)) ? Container(
      ...
      padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
      child: CustomButton(
        backgroundColor: (orderAmount < minimumAmount)
          ...
          onTap: () {
            ...
            if(isLogin){
              ...
              configModel!.minimumOrderValue!) {
                ...
                return AuthScreen(orderAmount: orderAmount,
                    totalWithoutDeliveryFee: totalWithoutDeliveryFee);
              },));
...
class CartListWidget extends StatelessWidget {
  ...
  const CartListWidget({Key? key,
    required this.cart,
    required this.addOns,
    required this.availableList}) : super(key: key);
